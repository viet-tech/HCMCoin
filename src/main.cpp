...
static const int64_t nTargetTimespan = 4 * 60; // nTargetTimespan = 960
...
const string strMessageMagic = "hochiminh Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // hochiminh: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // hochiminh: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
{
    if (pindexBest->nHeight < 6){
        int64_t nSubsidy = 1000000 * COIN; // ICO investors 3133700
        ...
        int64_t nSubsidy = 150 * COIN;  // 1500000 
        if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nSubsidy);
        return nSubsidy + nFees;
    }   else if (pindexBest->nHeight < 60000){
        int64_t nSubsidy = 100 * COIN; // 1000000 
        if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nSubsidy);
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 100001){
        int64_t nSubsidy = 50 * COIN; // 700000 
        if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nSubsidy);
        return nSubsidy + nFees;
    }
...
// hochiminh: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // hochiminh: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // hochiminh: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // hochiminh: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // hochiminh: track money supply and mint amount info
...
// hochiminh: total coin age spent in transaction, in the unit of coin-days.
...
// hochiminh: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // hochiminh: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // hochiminh: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // hochiminh: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // hochiminh: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // hochiminh: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // hochiminh: check proof-of-stake
    ...
    // hochiminh: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // hochiminh: getblocks may not obtain the ancestor block rejected
    ...
    // hochiminh: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "hochiminh", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "hochiminhcoin";
        CTransaction txNew;
        txNew.nTime = 1471145932;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1471145932;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2374474;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xd16cc33b2ffa8ea67d4efa73c47078c7b2b663a2385685558348f4112bc093bc"));
        block.print();
        ...
        // hochiminh: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // hochiminh: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // hochiminh: tell downloading node about the latest block if it's
