...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.33 * COIN;	// 33% annual interest
...
public:
    ...
    {
        // hochiminh: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // hochiminh: get transaction coin age
...
public:
    ...
    // hochiminh: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // hochiminh: two types of block: proof-of-work or proof-of-stake
    ...
    // hochiminh: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // hochiminh: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // hochiminh: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // hochiminh: block index flags
    enum  
    {
